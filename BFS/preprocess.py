...
def create_kb():
    entity_items=json.load(open('/data/wuwei/data/csqa_data/wikidata_dir/items_wikidata_n.json'))
    ...
    sub_predict_obj=json.load(open('/data/wuwei/data/csqa_data/wikidata_dir/wikidata_short_1.json'))
    ...
    sub_predict_obj=json.load(open('/data/wuwei/data/csqa_data/wikidata_dir/wikidata_short_2.json'))
    for idx in tqdm(sub_predict_obj,total=len(sub_predict_obj)):
            for x in sub_predict_obj[idx]:
                sub_predict_obj[idx][x]=set(sub_predict_obj[idx][x])        
            graph[get_id(idx)]['sub']=sub_predict_obj[idx]   

    obj_predict_sub=json.load(open('/data/wuwei/data/csqa_data/wikidata_dir/comp_wikidata_rev.json'))
    ...
    pickle.dump(graph,open('/data/wuwei/data/wikidata.pkl','wb'))
...
def create_entity_type():
    ...
    values: ids of type
    '''
    dic=json.load(open('/data/wuwei/data/csqa_data/wikidata_dir/par_child_dict.json'))
    max_id=0
    ...
    pickle.dump(par_dict, open('/data/wuwei/data/par_child.pkl', 'wb'))
    print("pickle down")
    for d in dic:
        for idx in dic[d]:# BUG?
            type_dict[get_id(idx)].append(d)
    pickle.dump(type_dict,open('/data/wuwei/data/child_par.pkl','wb'))
    print("pickle down")
    return type_dict
